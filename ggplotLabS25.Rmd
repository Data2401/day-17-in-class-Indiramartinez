...
```{r}
ggplot(heights) + geom_histogram(aes(x = height))
...
```{r}
ggplot(heights) + geom_density(aes(x = height))
```

Use a new argument in the `aes()`, `group =` to split this density by sex

```{r}
ggplot(heights) + geom_density(aes(x = height, group = sex))
```
...
```{r}
# color makes the outlines, 
ggplot(heights) + geom_density(aes(x = height, fill = sex))
...
```{r}
ggplot(heights) + geom_density(aes(x = height, fill = sex),alpha = .6)
...
```{r}
ggplot(heights) + geom_boxplot(aes(x = height, fill = sex),alpha = .6)
```
...
```{r}
head(heights)
heights %>% group_by(sex) %>% summarise(mean_data = mean(height), median_data = median(height))
```
...
heights %>% filter(sex == "Female") %>% summarise(tallest_female_group = max(height))

heights %>% filter(sex == "Male") %>% summarise(shortest_male_group = min(height))
